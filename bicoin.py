...
class Td:
    ...
    def news(self,title, text):
        try:
            url = 'https://sc.ftqq.com/SCU113133T49004715319059a6a61e3631cf3383775f5a35afefc0e.send?text=' + title + '&desp=' + text
    ...
    def bi_data(self):
        try:
            close_list = []
            url = 'https://api.hadax.com/market/history/kline?period='+self.cycle+'&size=17&symbol='+self.type+'usdt'
            request = requests.get(url=url).json()
    ...
    def td_out(self,close_list):
        ...
        td_num_set = []
        while i <= 13:
            if float(close_list[after]) >= float(close_list[front]):
                td_len += 1
                td_num_set.append(str(td_len))
            else:
                td_len = 0
...
if __name__ == '__main__':
    ...
    while True:
        lock.acquire()
        btc_class = Td()
        btc_process = threading.Thread(target=btc_class.time_set(type='btc'),daemon=True)
        btc_process.start()
        lock.release()
        time.sleep(1)
